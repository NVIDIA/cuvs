...
from cuvs_bench.backends.pylucene import _PyLuceneRuntime
...
CAGRA_BUILT_HNSW_BASE_LAYER_TEST_CODEC_CLASS = (
    "com.nvidia.cuvs.lucene.PyLuceneTestSupport$CagraBuiltHnswBaseLayerCodec"
)
CAGRA_BUILT_HNSW_THREE_LAYER_TEST_CODEC_CLASS = (
    "com.nvidia.cuvs.lucene.PyLuceneTestSupport$CagraBuiltHnswThreeLayerCodec"
)
...
HNSW_GRAPH_VERIFYING_EXACT_QUERY_CLASS = (
    "com.nvidia.cuvs.lucene.PyLuceneTestSupport$HnswGraphVerifyingExactQuery"
)
...
class IndexScenario:
    ...
    use_exact_hnsw_filter_query: bool = False
...
def initialize_pylucene_context(
    ...
    runtime = _PyLuceneRuntime.create(backend_config)
    runtime.attach_current_thread()
    class_class = runtime.Class
    codec_class = runtime.Codec

    # Resolve classpath failures before constructing codecs or indexes.
    class_class.forName(CPU_HNSW_TEST_CODEC_CLASS)
    class_class.forName(CAGRA_TEST_CODEC_CLASS)
    class_class.forName(CAGRA_BUILT_HNSW_BASE_LAYER_TEST_CODEC_CLASS)
    class_class.forName(CAGRA_BUILT_HNSW_THREE_LAYER_TEST_CODEC_CLASS)
    class_class.forName(CAGRA_TEST_QUERY_CLASS)
    class_class.forName(HNSW_GRAPH_VERIFYING_QUERY_CLASS)
    class_class.forName(HNSW_GRAPH_VERIFYING_EXACT_QUERY_CLASS)
    ...
    return PyLuceneContext(
        ...
        system_class=runtime.System,
        jarray=runtime.lucene.JArray,
        codec_cache={},
    )


def _codec_for_scenario(
...
def _writer_telemetry(codec: Any) -> dict[str, str]:
    ...
    if not separator or not payload.endswith(")"):
        raise RuntimeError(
            f"Malformed vector-format diagnostics: {description!r}"
        )
...
def _new_cagra_search_query(
    ...
    properties = {
        ...
        QUERY_PROPERTIES["target"]: ",".join(
            f"{value:.9g}" for value in target
        ),
...
def _new_hnsw_graph_verifying_query(
    ...
    *,
    ...
    with _temporary_system_properties(context.system_class, properties):
        reflected = context.class_class.forName(query_class).newInstance()
    return context.query_class.cast_(reflected)
...
def _new_vector_query(
    ...
    target = deterministic_float32_vector(
        query_document_id, scenario.dimensions
    )
    ...
    if scenario.expected_hnsw_m:
        return _new_hnsw_graph_verifying_query(
            ...
            filter_value,
            exact_search=(
                scenario.use_exact_hnsw_filter_query and bool(filter_field)
            ),
        )
    filter_query = (
