...
EXACT_FILTER_PATH_LABELS = {
    ExecutionPath.CPU_HNSW: "CPU HNSW build -> Lucene exact filtered search",
    ExecutionPath.GPU_CAGRA_BUILT_HNSW: (
        "GPU CAGRA build -> Lucene exact filtered search"
    ),
}
...
def _minimum_recall_from_env() -> float:
    value = float(
        os.environ.get("CUVS_BENCH_PYLUCENE_MIN_RECALL", DEFAULT_MIN_RECALL)
    )
    if not 0.0 <= value <= 1.0:
        raise ValueError(
            "CUVS_BENCH_PYLUCENE_MIN_RECALL must be between 0 and 1, "
            f"got {value}"
        )
    return value
...
def _suite_settings() -> SuiteSettings:
    requested_document_count = _positive_int_env(
        "CUVS_BENCH_PYLUCENE_ROWS", 2000
    )
    dimensions = _positive_int_env("CUVS_BENCH_PYLUCENE_DIMS", 32)
    top_k = _positive_int_env("CUVS_BENCH_PYLUCENE_TOPK", 20)
    if dimensions > 4096:
        raise ValueError(
            f"CUVS_BENCH_PYLUCENE_DIMS must be at most 4096, got {dimensions}"
        )
...
def _selective_document_filter_configuration(
    ...
    top_k: int,
    *,
    use_lucene_exact_search: bool = False,
    ...
    for segment_document_ids in segment_document_id_ranges(
        ...
        # Lucene 10.2 selects exact vector scoring up front when the accepted
        # document count is no greater than k.
    ...
    if query_document_id in accepted_document_ids:
        raise ValueError("selective filter must reject its query document")
...
def _cpu_hnsw_case(
    ...
    *,
    ...
    scenario = IndexScenario(
        ...
        use_exact_hnsw_filter_query=selective_filter,
        expected_hnsw_m=32,
    )
    return EndToEndCase(
        selector=selector,
...
DOCUMENT_FILTER_CASES = (
    _cpu_hnsw_case(
        "cpu-hnsw-lucene-exact-filter",
        selective_filter=True,
    ),
    _cagra_built_hnsw_case(
        "gpu-cagra-built-hnsw-lucene-exact-filter",
        selective_filter=True,
    ),
    _cagra_search_case(
...
def pylucene_context(pylucene_runtime_config) -> PyLuceneContext:
    return initialize_pylucene_context(
...
def _assert_index_metadata(case: EndToEndCase, result: IndexRun) -> None:
    expected_live_documents = case.scenario.document_count - len(
...
def _assert_execution_path(case: EndToEndCase, result: IndexRun) -> None:
    ...
    assert (
        telemetry.get("writerClass")
        == case.execution_path.expected_writer_class
    )
    ...
    if result.filtered_query_observation is not None:
        expected_filtered_query_class = (
            HNSW_GRAPH_VERIFYING_EXACT_QUERY_CLASS
            if case.scenario.use_exact_hnsw_filter_query
            else HNSW_GRAPH_VERIFYING_QUERY_CLASS
        )
        assert (
            result.filtered_query_observation.query_class
            == expected_filtered_query_class
...
def _assert_graph_configuration(case: EndToEndCase, result: IndexRun) -> None:
...
def _assert_search_results(
    ...
    for observation in result.query_observations:
        ...
        if observation.query_document_state is QueryDocumentState.SEARCHABLE:
        ...
        recall = len(set(hits) & set(expected_neighbors)) / len(
            expected_neighbors
        )
...
def _execution_path_label(case: EndToEndCase) -> str:
    return (
        ...
        if case.scenario.use_exact_hnsw_filter_query
        else case.execution_path.label
    )
