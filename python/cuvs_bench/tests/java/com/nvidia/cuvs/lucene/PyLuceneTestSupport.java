...
public final class PyLuceneTestSupport {
  ...
  public static final class HnswGraphVerifyingQuery extends KnnFloatVectorQuery {
    ...
    protected TopDocs approximateSearch(
        ...
        throws IOException {
      requireExpectedHnswGraph(context, getField(), expectedM);
    ...
    protected TopDocs exactSearch(
        ...
        throws IOException {
      throw new AssertionError("HNSW graph-verifying query must not use exact vector scoring");
    }
  }

  public static final class HnswGraphVerifyingExactQuery extends KnnFloatVectorQuery {

    private final int expectedM;

    public HnswGraphVerifyingExactQuery() {
      this(QueryProperties.fromSystemPropertiesForHnsw());
    }

    private HnswGraphVerifyingExactQuery(QueryProperties properties) {
      super(properties.field, properties.target, properties.k, properties.filter);
      expectedM = parsePositiveInt(QUERY_EXPECTED_HNSW_M_PROPERTY);
    }
    ...
    protected TopDocs approximateSearch(
        ...
        KnnCollectorManager knnCollectorManager) {
      throw new AssertionError(
          "HNSW exact-filter graph-verifying query must not use approximate traversal");
    }
  ...
  private static void requireExpectedHnswGraph(
      LeafReaderContext context, String fieldName, int expectedM) {
    KnnVectorsReader vectorsReader = vectorReaderForField(context, fieldName);
    ...
    var fieldInfo = context.reader().getFieldInfos().fieldInfo(fieldName);
    if (fieldInfo == null) {
      throw new AssertionError("HNSW field metadata has no field info for: " + fieldName);
    }

    int actualM = persistedHnswM(vectorsReader, fieldName, fieldInfo.number);
  ...
  public static final class CagraSearchQuery extends GPUKnnFloatVectorQuery {
    ...
    private CagraSearchQuery(QueryProperties properties) {
      ...
      requireConfiguredSearchWidth(properties.searchWidth);
    }
    ...
    private void requireConfiguredSearchWidth(int expectedSearchWidth) {
      try {
        Field searchWidthField = GPUKnnFloatVectorQuery.class.getDeclaredField("searchWidth");
        searchWidthField.setAccessible(true);
        int actualSearchWidth = searchWidthField.getInt(this);
        if (actualSearchWidth != expectedSearchWidth) {
          throw new AssertionError(
              "CAGRA search query expected retained searchWidth "
                  + expectedSearchWidth
                  + ", got "
                  + actualSearchWidth);
        }
      } catch (ReflectiveOperationException | RuntimeException exception) {
        throw new AssertionError("Unable to inspect retained CAGRA searchWidth", exception);
      }
    }
    ...
    private void requireCagraOnlyIndex(LeafReaderContext context) {
      ...
      long actualGraphDegree = cagraIndex.getGraphDegree();
