...
import time
import os
from common import get_search_params, get_nq_vec, load_gt_ids
from config import RECALL_NQ, RECALL_TOPK, RECALL_RES, RECALL_CALC_SCOPE, \
    RECALL_RES_TOPK

def save_search_res(collection_name, rand, results, search_param, query):
    if not os.path.exists(RECALL_RES):
        os.mkdir(RECALL_RES)
    file_name = os.path.join(RECALL_RES, collection_name + '_' + str(search_param) + '_' + str(query) + '_recall.txt')
...
def compute_recall(collection_name, query, results, search_param, rand, gt_ids, top_k_scope):
    ids = []
    for result in results:
        temp = []
        for res in result:
    ...
    for top_k in top_k_scope:
        ...
        fname = collection_name + '_' + str(search_param) + '_' + str(query) + "_" + str(top_k) + ".csv"
...
def recall(client, collection_name, search_param):
    ...
    recall_res = compute_recall(collection_name, RECALL_NQ, results, search_param, rand, RECALL_CALC_SCOPE)
    return recall_res
