...
std::vector<float> distances_exp = {0.03878258, 0.12472608, 0.04776672,
                                    0.15224178};
...
TEST(CagraHnswC, BuildSearch) {
  ...
  // Use NONE hierarchy since cuvsCagraSerializeToHnswlib creates a
  // base-layer-only index
  hnsw_params->hierarchy = NONE;
  cuvsHnswDeserialize(res, hnsw_params, "/tmp/cagra_hnswlib.index", 2,
                      L2Expanded, hnsw_index);

  // search index
  cuvsHnswSearchParams_t search_params;
  cuvsHnswSearchParamsCreate(&search_params);
  cuvsHnswSearch(res, search_params, hnsw_index, &queries_tensor,
                 &neighbors_tensor, &distances_tensor);

  // verify output
  ASSERT_TRUE(
      cuvs::hostVecMatch(neighbors_exp, neighbors, cuvs::Compare<uint64_t>()));
  ASSERT_TRUE(cuvs::hostVecMatch(distances_exp, distances,
                                 cuvs::CompareApprox<float>(0.001f)));
...
TEST(HnswC, BuildSearchHnswParams) {
  ...
  // create HNSW params with no ACE override
  cuvsHnswIndexParams_t hnsw_params;
  cuvsHnswIndexParamsCreate(&hnsw_params);
  ...
  // build index from HNSW parameters. Internally this uses CAGRA graph
  // construction.
  cuvsError_t build_status =
      cuvsHnswBuild(res, hnsw_params, &dataset_tensor, hnsw_index);
  ...
  // serialize and deserialize the HNSW-first index, then verify CPU search
  // still works
  auto hnsw_index_path =
      std::filesystem::temp_directory_path() /
      ("hnsw_build_api_" + std::to_string(getpid()) + ".index");
  cuvsError_t serialize_status =
      cuvsHnswSerialize(res, hnsw_index_path.c_str(), hnsw_index);
  ASSERT_EQ(serialize_status, CUVS_SUCCESS) << "cuvsHnswSerialize failed";
  ...
  cuvsError_t deserialize_status =
      cuvsHnswDeserialize(res, hnsw_params, hnsw_index_path.c_str(), 2,
                          L2Expanded, deserialized_index);
  ASSERT_EQ(deserialize_status, CUVS_SUCCESS) << "cuvsHnswDeserialize failed";

  std::fill(neighbors.begin(), neighbors.end(), uint64_t{0});
  std::fill(distances.begin(), distances.end(), 0.0f);

  search_status =
      cuvsHnswSearch(res, search_params, deserialized_index, &queries_tensor,
                     &neighbors_tensor, &distances_tensor);
  ASSERT_EQ(search_status, CUVS_SUCCESS)
      << "cuvsHnswSearch failed after deserialize";
  ...
  std::filesystem::remove(hnsw_index_path);
  cuvsResourcesDestroy(res);
}
...
TEST(HnswAceDiskC, BuildSerializeDeserializeSearch) {
  ...
  // build index using ACE with disk mode (index is serialized to disk by the
  // build function)
  cuvsError_t build_status =
      cuvsHnswBuild(res, hnsw_params, &dataset_tensor, hnsw_index);
  ASSERT_EQ(build_status, CUVS_SUCCESS) << "cuvsHnswBuild failed";
  ...
  cuvsError_t deserialize_status = cuvsHnswDeserialize(
      res, hnsw_params, "/tmp/hnsw_ace_disk_test/hnsw_index.bin", 2, L2Expanded,
      deserialized_index);
  ASSERT_EQ(deserialize_status, CUVS_SUCCESS) << "cuvsHnswDeserialize failed";
