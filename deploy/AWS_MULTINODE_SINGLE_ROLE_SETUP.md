# AWS single-role multinode setup

This guide sets up the three-node version of the OpenSearch GPU benchmark stack in `us-west-2`:
...
In the AWS Console, set the region selector to:

```text
US West (Oregon) us-west-2
```

Use this same region for S3, EC2, security groups, and the instances' IAM roles.
...
export DATASET_PATH=/data/opensearch-cuvs-datasets
...
- Keep all three instances in `us-west-2`.
- Prefer the same VPC and same Availability Zone for the first benchmark run.
