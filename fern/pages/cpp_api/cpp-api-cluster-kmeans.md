...
This overload supports out-of-core computation where the dataset resides on the host. Data is processed in batches, streaming from host to device. The batch size is controlled by `params.device_buffer_samples`. Multiple batches are automatically double-buffered. At least one auxiliary stream is required to overlap transfer and compute; without a stream pool, the fit remains correct but serialized. One auxiliary stream is sufficient. Pinned host memory is crucial for OOC performance: ordinary pageable or unregistered `mmap`-backed input degrades throughput rapidly.
