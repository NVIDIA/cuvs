 ...
 *
 * What is checked here is that a freshly compressed dataset builds a usable graph, that such an
 * index survives a trip through a file with its rows, and that the constraints above are rejected
