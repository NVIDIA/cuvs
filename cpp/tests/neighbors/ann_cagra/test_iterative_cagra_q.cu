 ...
 *
 * What is checked here is that a freshly compressed dataset builds a usable graph and that the
 * constraints above are rejected rather than accepted and quietly ignored.
 */
