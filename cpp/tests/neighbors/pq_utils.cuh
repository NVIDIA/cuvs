...
__global__ void decode_pq_dataset_kernel(data_t* const decoded_dataset_ptr,
                                         const uint32_t ldd,
...
template <class data_t, class math_t>
void decode_pq_dataset(raft::device_matrix_view<data_t, int64_t> decoded_dataset,
                       const cuvs::neighbors::device_pq_dataset<math_t, int64_t>& pq_dataset,
                       cudaStream_t cuda_stream)
{
  const auto dataset_size = decoded_dataset.extent(0);
  RAFT_EXPECTS(pq_dataset.data.extent(0) == dataset_size, "Dataset sizes mismatch");
  RAFT_EXPECTS(pq_dataset.pq_bits() == 8,
               "decode_pq_dataset currently only supports pq_bits == 8 (got %u)",
               pq_dataset.pq_bits());
  ...
  decode_pq_dataset_kernel<data_t, math_t>
    <<<grid_size, block_size, 0, cuda_stream>>>(decoded_dataset.data_handle(),
                                                decoded_dataset.stride(0),
                                                pq_dataset.vq_code_book.data_handle(),
                                                pq_dataset.vq_code_book.stride(0),
                                                pq_dataset.pq_code_book.data_handle(),
                                                pq_dataset.pq_len(),
                                                1u << pq_dataset.pq_bits(),
                                                pq_dataset.dim(),
                                                dataset_size,
                                                pq_dataset.data.data_handle(),
                                                pq_dataset.data.stride(0));
}
}  // namespace cuvs::neighbors
