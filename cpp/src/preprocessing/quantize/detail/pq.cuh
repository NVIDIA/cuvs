...
inline auto to_pq_params(const cuvs::preprocessing::quantize::pq::params& params)
  ...
  raft::mdspan<const DataT, raft::matrix_extent<int64_t>, raft::row_major, AccessorType> dataset)
{
  ...
  auto pq_params    = to_pq_params(filled_params);
  ...
  std::optional<raft::device_vector_view<uint32_t, int64_t>> vq_labels = std::nullopt)
{
  ...
  if (quantizer.params_quantizer.use_subspaces) {
    cuvs::neighbors::detail::process_and_fill_codes_subspaces<T, int64_t>(
      res,
      to_pq_params(quantizer.params_quantizer),
      dataset,
...
inline auto make_pq_params_from_pq(const cuvs::neighbors::pq_params& in_params,
              ...
              const DatasetT& dataset) -> cuvs::neighbors::device_pq_dataset<MathT, IdxT>
{
  ...
  auto pq_params = make_pq_params_from_pq(ps, dataset.extent(0));
  // Train codes
