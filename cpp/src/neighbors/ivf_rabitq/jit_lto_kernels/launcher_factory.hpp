  ...
  int ex_bits, bool with_ex, bool is_inner_product, SampleFilterType filter_type)
{
  ...
  if (with_ex) {
    if (is_inner_product) {
      planner.add_lut_block_sort_emit_topk_device_function<true, true>();
    } else {
      planner.add_lut_block_sort_emit_topk_device_function<true, false>();
    }
    add_ex_bits_device_functions(planner, ex_bits);
  } else {
    if (is_inner_product) {
      planner.add_lut_block_sort_emit_topk_device_function<false, true>();
    } else {
      planner.add_lut_block_sort_emit_topk_device_function<false, false>();
    }
  }
  return planner.get_launcher();
}
                                                               ...
                                                               SampleFilterType filter_type)
{
  ...
  if (with_ex) {
    if (is_inner_product) {
      planner.add_entrypoint<true, true>();
    } else {
      planner.add_entrypoint<true, false>();
    }
    add_ex_bits_device_functions(planner, ex_bits);
  } else {
    if (is_inner_product) {
      planner.add_entrypoint<false, true>();
    } else {
      planner.add_entrypoint<false, false>();
    }
  }
  return planner.get_launcher();
}
