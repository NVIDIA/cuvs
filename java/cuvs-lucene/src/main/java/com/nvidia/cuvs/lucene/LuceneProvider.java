...
public class LuceneProvider {
  ...
  private final Class<?> flatVectorsFormat;
  private final Class<?> hnswVectorsFormat;
  private final Class<?> hnswVectorsReader;
  private final Class<?> hnswVectorsWriter;
  private final Class<?> binaryQuantizedVectorsFormat;
  private final Class<?> hnswBinaryQuantizedVectorsFormat;
  private final Class<?> scalarQuantizedVectorsFormat;
  private final Class<?> hnswScalarQuantizedVectorsFormat;
  ...
  private LuceneProvider(String version) throws ClassNotFoundException {
    ...
    // Binary quantization was introduced in the lucene102 format family. General HNSW and scalar
    // quantization in Lucene 10.2 still use the lucene99 family, so the two provider versions have
    ...
    if (LUCENE_102_BINARY_FORMAT_VERSION.equals(version)) {
      ...
      hnswVectorsFormat = null;
      hnswVectorsReader = null;
      hnswVectorsWriter = null;
      scalarQuantizedVectorsFormat = null;
      hnswScalarQuantizedVectorsFormat = null;
      return;
    }
    ...
    binaryQuantizedVectorsFormat = null;
    hnswBinaryQuantizedVectorsFormat = null;
  }
   ...
   * binary-quantized format extends {@link KnnVectorsFormat} directly. Use {@link
   ...
   *
   * <p>The legacy API declared {@link FlatVectorsFormat} as its return type, but Lucene's HNSW
   * scalar-quantized format extends {@link KnnVectorsFormat} directly. Use {@link
   * #getLuceneHnswScalarQuantizedKnnVectorsFormatInstance(int, int)}. The legacy parameters are
   * ordered {@code (beamWidth, maxConn)}; the replacement uses {@code (maxConn, beamWidth)}.
   *
   * @deprecated The legacy return type cannot represent Lucene's HNSW format.
   */
  @Deprecated(since = "26.12", forRemoval = false)
  public FlatVectorsFormat getLuceneHnswScalarQuantizedVectorsFormatInstance(
    ...
    throw new UnsupportedOperationException(
        "Lucene HNSW scalar-quantized vectors require KnnVectorsFormat; use "
            + "getLuceneHnswScalarQuantizedKnnVectorsFormatInstance(maxConn, beamWidth); "
            + "the legacy parameter order was (beamWidth, maxConn)");
  }
}
