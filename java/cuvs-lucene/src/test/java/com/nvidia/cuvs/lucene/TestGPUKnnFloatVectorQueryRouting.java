...
/** Verifies explicit MULTI_KERNEL routing at the Lucene query boundary. */
...
public class TestGPUKnnFloatVectorQueryRouting extends LuceneTestCase {
  ...
  public void testExplicitMultiKernelUsesPerSegmentCagraSearch() throws Exception {
    ...
    try (Directory directory = newDirectory()) {
      ...
      try (DirectoryReader reader = DirectoryReader.open(directory)) {
        ...
        assertEverySegmentUsesOnlyCagra(reader);
        IndexSearcher searcher = newSearcher(reader);
        RoutingProbeQuery query = new RoutingProbeQuery(dataset[0]);

        ScoreDoc[] hits = searcher.search(query, TOP_K).scoreDocs;

        assertEquals(SEGMENT_COUNT, query.perSegmentSearchCount());
  ...
  private static void assertEverySegmentUsesOnlyCagra(DirectoryReader reader) {
    for (LeafReaderContext leaf : reader.leaves()) {
      ...
      assertNotNull("segment did not load GPU indexes", gpuReader.getCuvsIndexes());
  ...
  /** Records the protected per-leaf hook while retaining the real CAGRA search behavior. */
  private static final class RoutingProbeQuery extends GPUKnnFloatVectorQuery {
    private final AtomicInteger perSegmentSearchCount = new AtomicInteger();

    private RoutingProbeQuery(float[] target) {
      super(
          VECTOR_FIELD,
          target,
          TOP_K,
    ...
    protected TopDocs approximateSearch(
        ...
        throws IOException {
      perSegmentSearchCount.incrementAndGet();
      return super.approximateSearch(context, acceptDocs, visitedLimit, knnCollectorManager);
    }

    private int perSegmentSearchCount() {
      return perSegmentSearchCount.get();
    }
  }
}
